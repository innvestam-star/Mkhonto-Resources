...
grant select,insert,update on public.mrcip_ai_eval_suites to authenticated;
grant select,insert,update on public.mrcip_ai_eval_cases to authenticated;
grant select,insert,update on public.mrcip_ai_eval_runs to authenticated;
grant select,insert on public.mrcip_ai_eval_attempts to authenticated;
grant select,insert on public.mrcip_ai_eval_results to authenticated;
grant select,insert,update on public.mrcip_ai_provider_rates to authenticated;
grant select,insert,update on public.mrcip_ai_budget_policies to authenticated;
grant select,insert,update on public.mrcip_ai_provider_onboarding to authenticated;

grant select on public.mrcip_ai_eval_suites,public.mrcip_ai_eval_cases,public.mrcip_ai_eval_runs,public.mrcip_ai_provider_rates,public.mrcip_ai_budget_policies,public.mrcip_ai_provider_onboarding to service_role;
grant select,insert,update on public.mrcip_ai_eval_attempts to service_role;
grant select on public.mrcip_ai_eval_results to service_role;
...
revoke all on public.mrcip_ai_eval_suite_summary from public,anon,authenticated;
grant select on public.mrcip_ai_eval_suite_summary to authenticated;
...
revoke all on public.mrcip_ai_eval_run_summary from public,anon,authenticated;
grant select on public.mrcip_ai_eval_run_summary to authenticated;

create or replace view public.mrcip_ai_provider_production_readiness with (security_invoker=true) as
select cfg.organisation_id,cfg.id as provider_config_id,cfg.provider_key,cfg.model_name,cfg.status as provider_status,cfg.connection_status,cfg.credential_status,cfg.protected_data_allowed,
  o.id as onboarding_id,o.revision_no,o.status as onboarding_status,o.evaluation_run_id,o.prompt_version_id,o.rate_card_id,o.budget_policy_id,o.protected_data_approved,o.legal_data_handling_checked,o.security_review_checked,o.commercial_review_checked,o.decided_at,o.decided_by,
  er.passed as evaluation_passed,er.avg_overall_score,er.avg_groundedness_score,er.avg_citation_score,er.avg_policy_score,er.protection_leak_count,er.critical_failure_count,
  pr.status as rate_status,pr.currency as rate_currency,pr.input_cost_per_1m,pr.output_cost_per_1m,
  bp.status as budget_status,bp.monthly_budget,bp.daily_budget,bp.max_cost_per_execution,bp.max_output_tokens_per_execution,bp.max_executions_per_hour,bp.max_executions_per_day,bp.max_protected_executions_per_day,
  (cfg.status='active' and cfg.connection_status='verified' and cfg.credential_status='configured_external_secret' and o.status='production_approved' and er.passed is true and pr.status='active' and bp.status='active') as production_ready
from public.mrcip_ai_provider_configs cfg
left join lateral (select x.* from public.mrcip_ai_provider_onboarding x where x.organisation_id=cfg.organisation_id and x.provider_config_id=cfg.id order by x.revision_no desc limit 1) o on true
left join public.mrcip_ai_eval_runs er on er.organisation_id=cfg.organisation_id and er.id=o.evaluation_run_id
left join public.mrcip_ai_provider_rates pr on pr.organisation_id=cfg.organisation_id and pr.id=o.rate_card_id
left join public.mrcip_ai_budget_policies bp on bp.organisation_id=cfg.organisation_id and bp.id=o.budget_policy_id;
revoke all on public.mrcip_ai_provider_production_readiness from public,anon,authenticated;
grant select on public.mrcip_ai_provider_production_readiness to authenticated;
...
revoke all on public.mrcip_ai_budget_usage_summary from public,anon,authenticated;
grant select on public.mrcip_ai_budget_usage_summary to authenticated;